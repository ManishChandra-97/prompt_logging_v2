...
type Page = "Dashboard" | "Evaluation Workspace" | "Results" | "Settings";

const nav: Page[] = ["Dashboard", "Evaluation Workspace", "Results", "Settings"];
...
export default function Home() {
  ...
  return <div className="shell"><aside className="side"><div className="brand"><span className="brand-mark">◈</span><span>CPU EVALUATOR</span></div><div className="nav-label">WORKBENCH</div>{nav.map((item) => <button key={item} className={`nav ${page === item ? "active" : ""}`} onClick={() => setPage(item)}>{item}</button>)}<div className="side-foot"><span className="security-dot">●</span> local-first evaluation<br />CPU intake</div></aside><main className="content">{error && <div className="notice" style={{ marginBottom: 14 }}>{error} <button className="button tiny" onClick={() => setError(null)}>Dismiss</button></div>}{page === "Dashboard" && <Dashboard data={data} setPage={setPage} />}{page === "Evaluation Workspace" && <Workspace data={data} onRequest={request} openResults={openResults} />}{page === "Results" && <Results runs={resultRuns.length ? resultRuns : data.runs ?? []} />}{page === "Settings" && <Settings />}</main>{toast && <div className="toast">{toast}</div>}</div>;
}
...
function Workspace({ data, onRequest, openResults }: { data: Doc; onRequest: (path: string, options?: RequestInit, message?: string) => Promise<Doc | null>; openResults: (runs: Doc[]) => void }) {
...
function PromptTrace({ trace, legacyEvidence, incorrect }: { trace?: Doc; legacyEvidence?: string; incorrect: boolean }) {
  ...
  const renderInstruction = (item: Doc, index: number) => <div className="trace-item" key={`${item.id}-${index}`}><span className="trace-label">#{index + 1} Metric-criteria match</span><span className="trace-source">{item.sourceLabel} · line {item.lineStart} · relevance {item.relevance ?? "—"}</span><code>{item.text}</code></div>;
  return <div className="prompt-trace">{incorrect ? <><div className="trace-item expected"><span className="trace-label">Relevant metric criterion</span><span className="trace-source">{trace.promptLine?.sourceLabel ?? "No matching selected metric"}{trace.promptLine ? ` · line ${trace.promptLine.lineStart}` : ""}</span>{trace.promptLine && <code>{trace.promptLine.text}</code>}</div><div className="trace-heading">Top {Math.min(3, matches.length)} selected metric criteria related to the actual behavior</div>{matches.length ? matches.slice(0, 3).map(renderInstruction) : <span className="fine">No related wording was found in the selected metric criteria.</span>}<div className="trace-item improvement"><span className="trace-label">Suggested criterion refinement</span><p>{trace.improvement}</p></div></> : <span className="fine">The observed behavior matches the selected ideal action; no remediation source is needed.</span>}</div>;
}
