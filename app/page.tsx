...
function Workspace({ data, globals, nodes, onRequest, openResults }: { data: Doc; globals: Doc[]; nodes: Doc[]; onRequest: (path: string, options?: RequestInit, message?: string) => Promise<Doc | null>; openResults: (runs: Doc[]) => void }) {
  ...
  const [actualCsv, setActualCsv] = useState("");
  ...
  async function createDataset() {
    const result = await onRequest("/api/datasets", { method: "POST", body: JSON.stringify({ name, user_turn_csv: userCsv, actual_turn_csv: actualCsv || null }) }, "Dataset built. Review the caller and actual turns below.");
  ...
  return <><Header eyebrow="ONE WORKSPACE" title="Datasets, metrics & analysis" subtitle="Upload matching caller and agent turns, set the ideal behavior for each caller input, then score each complete conversation."><Badge value={data.judgeConfigured ? "LLM READY" : "LOCAL ONLY"} /></Header><section className="workspace-top"><div className="card panel"><h3>1. Caller turn data</h3><p className="section-desc">CSV headers supported: <span className="score">conversation_id, turn, user_turn</span>. Every row below gets a prompt-based next-step suggestion that you can override.</p><div className="drop" style={{ marginTop: 12 }}>Upload the caller-only turn file<br /><input type="file" accept=".csv,text/csv" onChange={(event) => readFile(event, setUserCsv)} /></div>{userCsv && <div className="notice safe" style={{ marginTop: 9 }}>Caller-turn CSV loaded · {userCsv.split("\n").length - 1} data lines</div>}</div><div className="card panel"><h3>2. Actual agent turns → evaluation evidence</h3><p className="section-desc">Use the same <span className="score">conversation_id</span> and <span className="score">turn</span>, plus <span className="score">actual_agent_turn</span> (agent, assistant, response also work).</p><div className="drop" style={{ marginTop: 12 }}>Upload the actual-call turn file<br /><input type="file" accept=".csv,text/csv" onChange={(event) => readFile(event, setActualCsv)} /></div>{actualCsv && <div className="notice safe" style={{ marginTop: 9 }}>Actual-turn CSV loaded · {actualCsv.split("\n").length - 1} data lines</div>}</div></section><section className="section card panel"><div className="form-grid"><div className="field"><label>Dataset name</label><input value={name} onChange={(event) => setName(event.target.value)} /></div><div className="field"><label>Existing dataset</label><Select value={datasetId} onChange={(value) => { setDatasetId(value); setWorking(null); }} items={datasets} empty="Choose an existing dataset" /></div></div><button className="button primary" disabled={!userCsv.trim()} style={{ marginTop: 12 }} onClick={() => void createDataset()}>Build editable ideal behavior table</button></section>{selectedDataset && <DatasetTable dataset={selectedDataset} onUpdate={async (next) => { const saved = await onRequest(`/api/datasets/${next.id}`, { method: "PUT", body: JSON.stringify(next) }); if (saved) setWorking(saved); }} />}<section className="section"><MetricBuilder name={metricName} prompt={metricPrompt} threshold={metricThreshold} setName={setMetricName} setPrompt={setMetricPrompt} setThreshold={setMetricThreshold} add={addMetric} /></section><section className="section card panel"><div className="section-header"><div><h2 className="section-title">Evaluation list</h2><p className="section-desc">Empty by default. Each selected metric is evaluated once against the whole conversation.</p></div><span className="pill">{evaluationMetrics.length} selected</span></div>{evaluationMetrics.length ? <div className="metric-list">{evaluationMetrics.map((metric: Doc) => <EvaluationMetric key={metric.id} metric={metric} onSave={async (next) => { const saved = await onRequest(`/api/metrics/${metric.id}`, { method: "PUT", body: JSON.stringify(next) }, "Metric version saved."); if (saved) setSelectedMetrics((items) => items.map((id) => id === metric.id ? saved.id : id)); }} onRemove={() => setSelectedMetrics((items) => items.filter((id) => id !== metric.id))} />)}</div> : <div className="empty">Add a metric and its evaluation prompt above. Nothing is preselected.</div>}</section><section className="section card panel"><div className="section-header"><div><h2 className="section-title">4. Analyze attached conversations</h2><p className="section-desc">Runs prompt tracing and flow reconstruction, then returns one 0–1 score and pass/fail result per metric for every complete conversation.</p></div>{selectedDataset && <span className="pill">{selectedDataset.actualConversationCount ?? 0} actual conversations attached</span>}</div><div className="form-grid"><div className="field"><label>Global prompt</label><Select value={globalId} onChange={setGlobalId} items={globals} empty="Active global prompt" /></div><div className="field"><label>Node prompt</label><Select value={nodeId} onChange={setNodeId} items={nodes} empty="Active node prompt" /></div></div><button className="button primary" disabled={running || !selectedDataset?.id} style={{ marginTop: 12 }} onClick={() => void analyze()}>{running ? "Analyzing conversations…" : "Analyze dataset"}</button>{!evaluationMetrics.length && <div className="notice info" style={{ marginTop: 10 }}>You can analyze prompt traces and flow without an evaluation metric, but weighted metric scores will remain not run until you add one.</div>}</section></>;
}

function DatasetTable({ dataset, onUpdate }: { dataset: Doc; onUpdate: (dataset: Doc) => Promise<void> }) {
  ...
  return <section className="section card panel"><div className="section-header"><div><h2 className="section-title">Choose ideal next behavior, then edit the reply</h2><p className="section-desc">The dropdown starts on the prompt-flow recommendation for this caller input. Changing it refreshes the editable ideal reply; your selection and edits are what analysis uses.</p></div><span className="pill">{draft.conversationCount} conversations</span></div><div className="table-wrap"><table className="table ideal-table"><thead><tr><th>Conversation ID</th><th>Turn</th><th>Caller input</th><th>Ideal next action</th><th>Ideal agent turn / tool event</th><th>Actual agent turn</th></tr></thead><tbody>{rows.map((row: Doc, index: number) => <tr key={`${row.conversationId}-${row.turn}-${index}`}><td><span className="score">{row.conversationId}</span></td><td>{row.turn}</td><td>{row.userTurn}</td><td><div className="field"><select value={row.idealAction || row.suggestedAction} onChange={(event) => updateRow(row.conversationId, row.id, "idealAction", event.target.value)}>{idealActions.map((action) => <option key={action} value={action}>{action.replaceAll("_", " ")}</option>)}</select><span className="fine">Prompt-flow recommendation: {row.suggestedAction.replaceAll("_", " ")}</span></div></td><td><textarea className="ideal-reply" value={row.idealResponse} onChange={(event) => updateRow(row.conversationId, row.id, "idealResponse", event.target.value)} /></td><td>{row.actual || <Badge value="MISSING" />}</td></tr>)}</tbody></table></div></section>;
}
...
function PromptTrace({ trace, legacyEvidence, incorrect }: { trace?: Doc; legacyEvidence?: string; incorrect: boolean }) {
  ...
  const line = trace.promptLine;
  const instruction = trace.problematicInstruction;
  const renderInstruction = (item: Doc | null, label: string) => item && <div className="trace-item"><span className="trace-label">{label}</span><span className="trace-source">{item.sourceLabel} · line {item.lineStart}</span><code>{item.text}</code></div>;
  return <div className="prompt-trace">{renderInstruction(line, "Actual prompt line")}{incorrect && <>{renderInstruction(instruction, "Potentially conflicting instruction")}<div className="trace-item improvement"><span className="trace-label">Improvement</span><p>{trace.improvement}</p></div></>}{!incorrect && <span className="fine">No prompt conflict detected for this action.</span>}</div>;
}
