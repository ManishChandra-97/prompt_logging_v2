...
type MetricFailure = { conversationId: string; why: string; potentialFix: string };
...
function failureDetail(run: Doc, metric: Doc): MetricFailure {
  ...
  const guidance = metric.potential_fix?.trim() || `You might need to review the active prompt instructions for “${metric.name}” and make the required behavior explicit, as the current guidance may be unclear or conflict with the expected response.`;
  const potentialFix = `This was given a fail due to: ${why} ${guidance}`;
  return { conversationId, why, potentialFix };
}
...
function PotentialFix({ failures }: { failures: MetricFailure[] }) {
  if (!failures.length) return <span className="fine">—</span>;
  const fixes = [...new Set(failures.map((failure) => failure.potentialFix))];
  return <div className="potential-fix">{fixes.slice(0, 3).map((fix) => <p key={fix}>{fix}</p>)}{fixes.length > 3 && <span className="fine">+{fixes.length - 3} more</span>}</div>;
}
